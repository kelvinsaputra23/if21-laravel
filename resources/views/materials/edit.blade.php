...
@section('content')
    ...
    <form action="{{ route('materials.update', $material->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="description" name="description" rows="5">{{ old('description', $material->description) }}</textarea>
        </div>
        <div class="mb-3">
            ...
            <select class="form-control" id="course_id" name="course_id" required>
                ...
                @foreach($courses as $course) {{-- Asumsi $courses dikirim dari controller --}}
                    <option value="{{ $course->id }}" {{ old('course_id', $material->course_id) == $course->id ? 'selected' : '' }}>{{ $course->nama }}</option>
                @endforeach
            </select>
        </div>
